...
mod tests {
    ...
    fn toggle_theme_flips_mode_and_returns_outcome() {
        ...
        let outcome = state.update(OnboardingMessage::ToggleTheme);
        assert_eq!(outcome, OnboardingOutcome::ThemeToggled(ThemeMode::Light));
    ...
    fn skip_yields_skipped_outcome() {
        ...
        let outcome = state.update(OnboardingMessage::Skipped);
        assert_eq!(outcome, OnboardingOutcome::Skipped);
    }

    #[test]
    fn mark_completed_persists_via_strategy() {
        let persistence: Arc<dyn OnboardingPersistence> =
            Arc::new(InMemoryOnboardingPersistence::new());
        mark_completed(&persistence).unwrap();
        assert!(persistence.is_completed());
    }
}
